...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
        ...
        self.w = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.latent_dim, 1)).cuda())
        self.w_user = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.latent_dim, 1)).cuda())
    ...
    def getUsersRating(self, users):
        ...
        preds = torch.matmul(users_emb, items_emb.t())
        pos_items_stop = items_emb
        users_stop =  users_emb
        self.pos_item_scores = torch.mm(pos_items_stop, self.w)
        self.user_scores =  torch.mm(users_stop, self.w_user)
    
        preds = (preds-self.config['rubi_c'])*torch.sigmoid(self.pos_item_scores).T*torch.sigmoid(self.user_scores)
            

        return preds, rating_orig
    ...
    def bpr_loss(self, users, pos, neg):
        ...
        pos_scores = torch.mul(users_emb, pos_emb) # BXd
        pos_scores = torch.sum(pos_scores, dim=1)  # BX1 (user_u, item_i의 점수.)
        neg_scores = torch.mul(users_emb, neg_emb)
        neg_scores = torch.sum(neg_scores, dim=1)
        
        pos_items_stop = pos_emb
        neg_items_stop = neg_emb
        users_stop = users_emb
        self.pos_item_scores = torch.mm(pos_items_stop, self.w)
        self.neg_item_scores = torch.mm(neg_items_stop, self.w)
        self.user_scores =  torch.mm(users_stop, self.w_user)
        
        # first branch
        # fusion
        pos_scores = pos_scores.unsqueeze(1)*torch.sigmoid(self.pos_item_scores)*torch.sigmoid(self.user_scores)
        neg_scores = neg_scores.unsqueeze(1)*torch.sigmoid(self.neg_item_scores)*torch.sigmoid(self.user_scores)
        ...
        self.mf_loss_ori = (-torch.log(torch.sigmoid(pos_scores)+1e-10)-torch.log(1-torch.sigmoid(neg_scores)+1e-10)).mean()
        # second branch
        self.mf_loss_item = (-torch.log(torch.sigmoid(self.pos_item_scores)+1e-10)-torch.log(1-torch.sigmoid(self.neg_item_scores)+1e-10)).mean()
        # third branch
        self.mf_loss_user = (-torch.log(torch.sigmoid(self.user_scores)+1e-10)-torch.log(1-torch.sigmoid(self.user_scores)+1e-10)).mean()
        # unify
        mf_loss = self.mf_loss_ori + self.config['alpha']*self.mf_loss_item + self.config['beta']*self.mf_loss_user
