...
def test_one_batch_grp(X, user_grp_batch, item_grp):
    sorted_items = X[0].numpy() # BX20
    groundTrue = X[1]
    top_K_items_grp = item_grp[sorted_items] # item groups that are recommended as top K
    r = utils.getLabel(groundTrue, sorted_items)
    recall_lst_ui, ndcg_lst_ui = np.zeros((5,5)), np.zeros((5,5))
    recall_lst_u, ndcg_lst_u, recall_lst_i, ndcg_lst_i = np.zeros(5), np.zeros(5), np.zeros(5), np.zeros(5)
    k=20
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0, use_orig=False):
    ...
    with torch.no_grad():
        ...
        if multicore == 1:
            ...
            pre_results_grp = pool.map(test_one_batch_grp, (X, user_grp_lst, item_grp))
        else:
            pre_results, pre_results_grp = [], []
            for (i,x) in enumerate(X):
                pre_results.append(test_one_batch(x))
                pre_results_grp.append(test_one_batch_grp(x, user_grp_lst[i], item_grp))
        ...
        top_K_items = np.concatenate(rating_list)
        top_K_items_grp = dataset.item_grp[top_K_items].squeeze() # item groups that are recommended as top K
        # metric 1: Rec_Ratio and C_ratio
        c_ratio = utils.C_Ratio(item_grp[top_K_items[users]])
