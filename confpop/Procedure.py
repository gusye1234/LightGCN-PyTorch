...
def train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    for (batch_i,
        ...
        cri, loss_f = get_loss.stageOne(batch_users, batch_pos, batch_neg)
        aver_loss += cri
        aver_loss_f += loss_f
        if world.tensorboard:
            w.add_scalar(f'BPRLoss/BPR', cri, epoch * int(len(users) / world.config['bpr_batch_size']) + batch_i)
    aver_loss = aver_loss / total_batch
    aver_loss_f = aver_loss_f / total_batch
    ...
    wandb.log({
        ...
        'loss_f': round(aver_loss_f, 3)
    })
    return f"loss{aver_loss:.3f}-loss_f{aver_loss_f:.3f}-{time_info}"
...
def Test(dataset, Recmodel, epoch, w=None, multicore=0, weight=0, use_orig=False, mode='confpop', is_test=False):
    ...
    conformity, popularity, log_popularity = dataset.user_conf_zdist, dataset.item_pop_zdist, dataset.item_pop_logdist
    ...
    with torch.no_grad():
        ...
        for batch_users in utils.minibatch(users, batch_size=u_batch_size):
            ...
            if mode == 'confpop':
                rating = rating.cpu() + weight*torch.matmul(conformity[batch_users], popularity.T)
            elif mode == 'pop':
                rating = rating.cpu() - weight*(torch.pow(log_popularity, 2)).T
        ...
        if is_test:
            ...
            #utils.get_sig_mean(rating, user_grp[batch_users], item_grp)

            # metric4: conformity rate
            #conf_rate = utils.ConformityRate(user_conf_norm, top_K_items_pop)
