...
class BPRLoss:
    ...
    def stageOne(self, users, pos, neg):
        loss, reg_loss, loss_f = self.model.bpr_loss(users, pos, neg)
        reg_loss = reg_loss*self.weight_decay
        loss_f = loss_f*self.alpha
        loss = loss + reg_loss + loss_f

        self.opt.zero_grad()
        loss.backward()
        self.opt.step()

        return loss.cpu().item(), loss_f.cpu().item()
...
class BCELoss:
    ...
    def stageOne(self, users, pos, neg):
        loss, reg_loss, loss_f = self.model.bce_loss(users, pos, neg)
        reg_loss = reg_loss*self.weight_decay
        loss_f = loss_f*self.alpha
        loss = loss + reg_loss + loss_f
...
def get_sig_mean(rating, user_attrs, item_attrs):
