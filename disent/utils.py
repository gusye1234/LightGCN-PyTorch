...
class BPRLoss:
    ...
    def stageOne(self, users, pos, neg):
        loss1 = 0.
        loss, reg_loss, clf_loss = self.model.bpr_loss(users, pos, neg)
        reg_loss = reg_loss*self.weight_decay
        loss = loss + reg_loss
        
        _, item_emb = self.model.computer()
        ...
        # cos
        loss2 = 0
        pop_emb = self.model.pop_emb
        for i in range(1,item_emb.shape[1]):
            loss2 += pearson(item_emb[:,i], pop_emb)

        loss = loss + clf_loss * 0.4 + loss1 * 0.4 + loss2 * 0.0001 * 0.4

        self.opt.zero_grad()
        loss.backward()
        self.opt.step()

        return loss.cpu().item(), clf_loss.cpu().item(), loss1.cpu().item(), loss2.cpu().item()
...
def C_Ratio(top_K_items_grp):
    ...
    c_ratio = torch.mean((top_K_items_grp == hot_grp_idx).sum(axis=1) / k)
    return c_ratio.item()
