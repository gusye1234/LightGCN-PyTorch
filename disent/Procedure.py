...
def train_original(dataset, recommend_model, loss_class, epoch, neg_k=1, w=None):
    ...
    aver_loss, aver_clf_loss, aver_loss1, aver_loss2 = 0., 0., 0., 0.
    ...
    for (batch_i,
        ...
        loss, clf_loss, loss1, loss2 = get_loss.stageOne(batch_users, batch_pos, batch_neg)
        aver_loss += loss
        aver_clf_loss += clf_loss
        aver_loss1 += loss1
        aver_loss2 += loss2
        if world.tensorboard:
            w.add_scalar(f'BPRLoss/BPR', loss, epoch * int(len(users) / world.config['bpr_batch_size']) + batch_i)
    aver_loss = aver_loss / total_batch
    aver_clf_loss = aver_clf_loss / total_batch
    ...
    wandb.log({
        ...
        'clf_loss': round(aver_clf_loss, 3),
        'disent_loss': round(aver_loss1, 3),
        'sim_loss': round(aver_loss2, 3)
    })
    return f"loss{aver_loss:.3f}-clf_loss{aver_clf_loss:.3f}-disent_loss{aver_loss1:.3f}-sim_loss{aver_loss2:.3f}-{time_info}"
