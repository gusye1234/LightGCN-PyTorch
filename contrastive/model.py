...
class PureMF(BasicModel):
    def __init__(self, 
        ...
        self.f = nn.Sigmoid()
        self.user_conf_zdist = dataset.user_conf_zdist.to(world.device).to(torch.float32)
        self.item_pop_zdist = dataset.item_pop_zdist.to(world.device).to(torch.float32)
        self.config = config
        self.__init_weight()
    ...
    def bpr_loss(self, users, pos, neg):
        ...
        # fair cl loss
        conf_batch = self.user_conf_zdist[users].squeeze()
        pos_pop_batch = self.item_pop_zdist[pos].squeeze()
        neg_pop_batch = self.item_pop_zdist[neg].squeeze()

        # (popularity=0인 positive, )
        #fair_pos_score = torch.log(torch.exp(u_emb[conf_batch<0] @ u_emb.T / self.temp).sum(1) + 1e-8).mean() 
        fair_pos_score = torch.log(torch.exp(torch.clamp(pos_emb[pos_pop_batch<0] @ pos_emb.T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 
        # (popularity=1인 positive, )
        #fair_pos_score += torch.log(torch.exp(u_emb[conf_batch>=0] @ u_emb.T / self.temp).sum(1) + 1e-8).mean() 
        fair_pos_score += torch.log(torch.exp(torch.clamp(pos_emb[pos_pop_batch>=0] @ pos_emb.T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 

        # (popularity=0인 negative, )
        fair_pos_score += torch.log(torch.exp(torch.clamp(neg_emb[neg_pop_batch<0] @ neg_emb.T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 
        # (popularity=1인 negative, )
        fair_pos_score += torch.log(torch.exp(torch.clamp(neg_emb[neg_pop_batch>=0] @ neg_emb.T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 

        # popularity=0인 positive
        #fair_neg_score = torch.log(torch.exp(u_emb[conf_batch<0] @ u_emb.T / self.temp).sum(1) + 1e-8).mean() 
        fair_neg_score = torch.log(torch.exp(torch.clamp(pos_emb[pos_pop_batch<0] @ neg_emb[neg_pop_batch<0].T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 
        # (popularity=1인 positive, )
        #fair_neg_score += torch.log(torch.exp(u_emb[conf_batch>=0] @ u_emb.T / self.temp).sum(1) + 1e-8).mean() 
        fair_neg_score += torch.log(torch.exp(torch.clamp(pos_emb[pos_pop_batch>=0] @ neg_emb[neg_pop_batch>=0].T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 

        # (popularity=0인 negative, )
        fair_neg_score += torch.log(torch.exp(torch.clamp(neg_emb[neg_pop_batch<0] @ pos_emb[pos_pop_batch<0].T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 
        # (popularity=1인 negative, )
        fair_neg_score += torch.log(torch.exp(torch.clamp(neg_emb[neg_pop_batch>=0] @ pos_emb[pos_pop_batch>=0].T / self.config['temp'], -5.0, 5.0)).sum(1) + 1e-8).mean() 
        #negative_sample =  # 다른 타겟 class이면서 같은 sensitive attribute인 것.
        ...
        loss_f = -fair_pos_score + fair_neg_score

        return loss, reg_loss, loss_f
